...
#define PACKAGE_NAME "team.maodie.aimbot"
#define SERVER_NAME "aimbot_svc"
#define SERVER_CLASS_PATH "team.maodie.aimbot.svc.server.AimSvcService"
...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    ARG_PUSH_FMT(argv, "-Daimbot.library.path=%s", lib_path)
...
static void start_server(const char *path, const char *main_class, const char *process_name) {
    ...
    switch (pid) {
        ...
        default: {
            printf("info: aimbot_svc pid is %d\n", pid);
            printf("info: aimbot_svc_starter exit with 0\n");
            exit(EXIT_SUCCESS);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    uid_t uid = getuid();
    if (uid != 0 && uid != 2000) {
        perrorf("fatal: run AimSvc from non root nor adb user (uid=%d).\n", uid);
        exit(EXIT_FATAL_UID);
    }
    ...
    foreach_proc([](pid_t pid) {
        ...
        if (kill(pid, SIGKILL) == 0)
            ...
            perrorf("fatal: can't kill %d, please try to stop existing AimSvc from app first.\n", pid);
