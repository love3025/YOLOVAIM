...
public class EmbeddedShizukuInjectorClient implements TouchInjectorInterface {
    private static final String TAG = "EmbeddedSvcInjector";
    ...
    public EmbeddedShizukuInjectorClient(Context context) {
        this.context = context;
    }

    @SuppressLint("PrivateApi")
    public void connect(InjectorCallback callback) {
        Log.d(TAG, "Attempting embedded AimSvc connection...");

        boolean pingOk = AimSvc.pingBinder();
        Log.d(TAG, "AimSvc pingBinder: " + pingOk);

        if (!pingOk) {
            Log.e(TAG, "embedded AimSvc not running");
            callback.onError("Embedded service not running");
            return;
        }

        int permResult = AimSvc.checkSelfPermission();
        Log.d(TAG, "AimSvc permission check: " + permResult + " (granted=" + android.content.pm.PackageManager.PERMISSION_GRANTED + ")");
        if (permResult != android.content.pm.PackageManager.PERMISSION_GRANTED) {
            Log.e(TAG, "AimSvc permission not granted: " + permResult);
            callback.onError("Embedded service permission not granted");
            return;
        }

        try {
            AimSvc.UserServiceArgs args = new AimSvc.UserServiceArgs(
            ...
            android.content.ServiceConnection conn = new android.content.ServiceConnection() {
                ...
                public void onServiceDisconnected(ComponentName name) {
                    Log.w(TAG, "embedded service disconnected");
                    connected = false;
                    remoteService = null;
            ...
            Log.d(TAG, "Calling AimSvc.bindUserService...");
            AimSvc.bindUserService(args, conn);
            Log.d(TAG, "bindUserService called, waiting for callback...");

            mainHandler.postDelayed(() -> {
                if (!connected) {
                    Log.e(TAG, "Embedded AimSvc connection timeout after 10s");
                    connected = false;
    ...
    public void tap(int x, int y) {
        if (remoteService != null) {
            try { remoteService.tap(x, y); } catch (Exception e) { Log.e(TAG, "tap: " + e.getMessage()); }
        }
    }
    ...
    public void blockPhysicalTouch() {
        if (remoteService != null) {
            try { remoteService.blockPhysicalTouch(); } catch (Exception e) { Log.e(TAG, "blockPhysicalTouch: " + e.getMessage()); }
        }
    }

    public void unblockPhysicalTouch() {
        if (remoteService != null) {
            try { remoteService.unblockPhysicalTouch(); } catch (Exception e) { Log.e(TAG, "unblockPhysicalTouch: " + e.getMessage()); }
        }
    }
    ...
    public void disconnect() {
        ...
        remoteService = null;
        try {
            AimSvc.unbindUserService(
                new AimSvc.UserServiceArgs(
                    new ComponentName(context, RemoteInjectorService.class))
                    .processNameSuffix("injector"),
                null, true
            );
