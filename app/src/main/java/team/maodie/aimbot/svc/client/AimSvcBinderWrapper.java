 ...
 */
public class AimSvcBinderWrapper implements IBinder {

    private final IBinder original;

    public AimSvcBinderWrapper(@NonNull IBinder original) {
    ...
    public boolean transact(int code, @NonNull Parcel data, @Nullable Parcel reply, int flags) throws RemoteException {
        ...
        try {
            newData.writeInterfaceToken(AimSvcApiConstants.BINDER_DESCRIPTOR);
            ...
            if (atLeast13) {
                AimSvc.transactRemote(newData, reply, 0);
            } else {
                AimSvc.transactRemote(newData, reply, flags);
            }
        } finally {
            newData.recycle();
        }
