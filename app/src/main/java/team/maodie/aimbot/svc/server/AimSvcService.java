...
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.ATTACH_APPLICATION_API_VERSION;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.ATTACH_APPLICATION_PACKAGE_NAME;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_PERMISSION_GRANTED;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_SERVER_PATCH_VERSION;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_SERVER_SECONTEXT;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_SERVER_UID;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_SERVER_VERSION;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.BIND_APPLICATION_SHOULD_SHOW_REQUEST_PERMISSION_RATIONALE;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.REQUEST_PERMISSION_REPLY_ALLOWED;
import static team.maodie.aimbot.svc.client.AimSvcApiConstants.REQUEST_PERMISSION_REPLY_IS_ONETIME;
import static team.maodie.aimbot.svc.server.AimSvcServerConstants.MANAGER_APPLICATION_ID;
import static team.maodie.aimbot.svc.server.AimSvcServerConstants.PERMISSION;
...
import team.maodie.aimbot.svc.common.util.BuildUtils;
import team.maodie.aimbot.svc.common.util.OsUtils;
import team.maodie.aimbot.svc.aidl.IShizukuApplication;
...
import team.maodie.aimbot.svc.rish.RishConfig;
import team.maodie.aimbot.svc.client.AimSvcApiConstants;
import team.maodie.aimbot.svc.server.api.IContentProviderUtils;
import team.maodie.aimbot.svc.server.util.HandlerUtil;
import team.maodie.aimbot.svc.server.util.UserHandleCompat;

public class AimSvcService extends Service<AimSvcUserServiceManager, AimSvcClientManager, AimSvcConfigManager> {

    public static void main(String[] args) {
        DdmHandleAppName.setAppName("aimbot_svc", 0);
        RishConfig.setLibraryPath(System.getProperty("aimbot.library.path"));
    ...
    private final AimSvcClientManager clientManager;
    private final AimSvcConfigManager configManager;
    private final int managerAppId;

    public AimSvcService() {
        super();
        ...
        if (ai == null) {
            System.exit(AimSvcServerConstants.MANAGER_APP_NOT_FOUND);
        }
    ...
    @Override
    public AimSvcUserServiceManager onCreateUserServiceManager() {
        return new AimSvcUserServiceManager();
    }

    @Override
    public AimSvcClientManager onCreateClientManager() {
        return new AimSvcClientManager(getConfigManager());
    }

    @Override
    public AimSvcConfigManager onCreateConfigManager() {
        return new AimSvcConfigManager();
    }
    ...
    private int checkCallingPermission() {
        try {
            return ActivityManagerApis.checkPermission(AimSvcServerConstants.PERMISSION,
    ...
    public void attachApplication(IShizukuApplication application, Bundle args) {
        ...
        int replyServerVersion = AimSvcApiConstants.SERVER_VERSION;
        if (apiVersion == -1) {
            // AimSvcBinderWrapper has adapted API v13 in dev.team.maodie.aimbot.svc:api 12.2.0, however
        ...
        reply.putInt(BIND_APPLICATION_SERVER_PATCH_VERSION, AimSvcApiConstants.SERVER_PATCH_VERSION);
        if (!isManager) {
    ...
    public void showPermissionConfirmation(int requestCode, @NonNull ClientRecord clientRecord, int callingUid, int callingPid, int userId) {
        ...
        Intent intent = new Intent(AimSvcServerConstants.REQUEST_PERMISSION_ACTION)
    ...
    private int  getFlagsForUidInternal(int uid, int mask, boolean allowRuntimePermission) {
        AimSvcConfig.PackageEntry entry = configManager.find(uid);
        if (entry != null) {
            ...
            for (PackageInfo pi : PackageManagerApis.getInstalledPackagesNoThrow(PackageManager.GET_META_DATA | PackageManager.GET_PERMISSIONS, user)) {
                ...
                if (flags != 0) {
                    list.add(pi);
                        ...
                        && pi.applicationInfo.metaData.getBoolean("team.maodie.aimbot.svc.client.V3_SUPPORT", false)
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        if (code == AimSvcServerConstants.BINDER_TRANSACTION_getApplications) {
            data.enforceInterface(AimSvcApiConstants.BINDER_DESCRIPTOR);
    ...
    static void sendBinderToUserApp(Binder binder, String packageName, int userId, boolean retry) {
        ...
        try {
            ...
            extra.putParcelable("team.maodie.aimbot.svc.intent.extra.BINDER", new BinderContainer(binder));
