...
public class BinderSender {
    ...
    private static final String PERMISSION_MANAGER = "team.maodie.aimbot.permission.AIMBOT_MANAGER";
    private static final String PERMISSION = "team.maodie.aimbot.permission.AIMBOT_API";

    private static AimSvcService sShizukuService;
    ...
    private static void sendBinder(int uid, int pid) throws RemoteException {
        ...
        for (String packageName : packages) {
            ...
            if (ArraysKt.contains(pi.requestedPermissions, PERMISSION_MANAGER)) {
                ...
                if (granted) {
                    AimSvcService.sendBinderToManger(sShizukuService, userId);
                    return;
                }
            } else if (ArraysKt.contains(pi.requestedPermissions, PERMISSION)) {
                AimSvcService.sendBinderToUserApp(sShizukuService, packageName, userId);
                return;
            }
        }
    }

    public static void register(AimSvcService shizukuService) {
