...
import team.maodie.aimbot.svc.aidl.IShizukuService;
...
public class AimSvc {
    ...
    private static boolean attachApplicationV13(IBinder binder, String packageName) throws RemoteException {
        ...
        args.putInt(ATTACH_APPLICATION_API_VERSION, AimSvcApiConstants.SERVER_VERSION);
        args.putString(ATTACH_APPLICATION_PACKAGE_NAME, packageName);

        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        try {
            data.writeInterfaceToken("team.maodie.aimbot.svc.aidl.IShizukuService");
     ...
     * <p>
     * Use {@link AimSvcBinderWrapper} to wrap the original binder.
     *
     * @see AimSvcBinderWrapper
     */
    public static void transactRemote(@NonNull Parcel data, @Nullable Parcel reply, int flags) {
        try {
            requireService().asBinder().transact(AimSvcApiConstants.BINDER_TRANSACTION_transact, data, reply, flags);
     ...
     */
    private static AimSvcRemoteProcess newProcess(@NonNull String[] cmd, @Nullable String[] env, @Nullable String dir) {
        try {
            return new AimSvcRemoteProcess(requireService().newProcess(cmd, env, dir));
        } catch (RemoteException e) {
            throw rethrowAsRuntimeException(e);
        }
    }
    ...
    public static int getLatestServiceVersion() {
        return AimSvcApiConstants.SERVER_VERSION;
    }
    ...
    public static class UserServiceArgs {
        ...
        private Bundle forAdd() {
            ...
            options.putParcelable(AimSvcApiConstants.USER_SERVICE_ARG_COMPONENT, componentName);
            options.putBoolean(AimSvcApiConstants.USER_SERVICE_ARG_DEBUGGABLE, debuggable);
            options.putInt(AimSvcApiConstants.USER_SERVICE_ARG_VERSION_CODE, versionCode);
            options.putBoolean(AimSvcApiConstants.USER_SERVICE_ARG_DAEMON, daemon);
            options.putBoolean(AimSvcApiConstants.USER_SERVICE_ARG_USE_32_BIT_APP_PROCESS, use32BitAppProcess);
            options.putString(AimSvcApiConstants.USER_SERVICE_ARG_PROCESS_NAME,
                    Objects.requireNonNull(processName, "process name suffix must not be null"));
            if (tag != null) {
                options.putString(AimSvcApiConstants.USER_SERVICE_ARG_TAG, tag);
            }
            return options;
        }
        ...
        private Bundle forRemove(boolean remove) {
            ...
            options.putBoolean(AimSvcApiConstants.USER_SERVICE_ARG_REMOVE, remove);
            return options;
        }
    }
    ...
    public static void bindUserService(@NonNull UserServiceArgs args, @NonNull ServiceConnection conn) {
        AimSvcServiceConnection connection = AimSvcServiceConnections.get(args);
    ...
    public static int peekUserService(@NonNull UserServiceArgs args, @NonNull ServiceConnection conn) {
        ...
        try {
            ...
            bundle.putBoolean(AimSvcApiConstants.USER_SERVICE_ARG_NO_CREATE, true);
            result = requireService().addUserService(connection, bundle);
        } catch (RemoteException e) {
            throw rethrowAsRuntimeException(e);
        }

        boolean atLeast13 = !AimSvc.isPreV11() && AimSvc.getVersion() >= 13;
        if (atLeast13) {
            return result;
        }
    ...
    public static void unbindUserService(@NonNull UserServiceArgs args, @Nullable ServiceConnection conn, boolean remove) {
        if (remove) {
            ...
            /*
             * When unbindUserService remove=false is called, although the AimSvcServiceConnection
             * instance is removed from AimSvcServiceConnections, it still exists (since its a Binder),
             ...
             * This will not kill the service, but will remove the AimSvcServiceConnection instance
             * from the server.
             */
            if (AimSvc.getVersion() >= 14 || AimSvc.getVersion() == 13 && AimSvc.getServerPatchVersion() >= 4) {
                try {
            ...
            AimSvcServiceConnections.remove(connection);
        }
    }
