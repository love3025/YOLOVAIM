...
import team.maodie.aimbot.svc.rish.RishService;
import team.maodie.aimbot.svc.client.AimSvcApiConstants;
import team.maodie.aimbot.svc.server.api.RemoteProcessHolder;
import team.maodie.aimbot.svc.server.util.Logger;
import team.maodie.aimbot.svc.server.util.OsUtils;
...
public abstract class Service<
    ...
    public Service() {
        RishConfig.init(AimSvcApiConstants.BINDER_DESCRIPTOR, 30000);
    ...
    public final int addUserService(IShizukuServiceConnection conn, Bundle options) {
        ...
        ClientRecord clientRecord = clientManager.findClient(callingUid, callingPid);
        if (clientRecord == null) {
            callingApiVersion = AimSvcApiConstants.SERVER_VERSION;
        } else {
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        if (code == AimSvcApiConstants.BINDER_TRANSACTION_transact) {
            ...
            args.putString(AimSvcApiConstants.ATTACH_APPLICATION_PACKAGE_NAME, packageName);
            args.putInt(AimSvcApiConstants.ATTACH_APPLICATION_API_VERSION, -1);
