...
Package: `io.github.love3025.yolovaim` | minSdk=31 | targetSdk=35 | compileSdk=36
...
└── JniCallBack.kt       — JNI bridge → libyolovaim.so
        └── yolovaim.cpp   — TFLite + QNN HTP inference
...
- Three native targets: `yolovaim` (inference), `uinput_inject` (Shizuku path), `root_daemon` (su path)
...
- `yolovaim.cpp` JNI methods: `init()`, `detect()`, `setConfidence()`, `getBackend()`, `release()`
- Input format auto-detected: NCHW `[1,3,H,W]` or NHWC `[1,H,W,3]`
- Output format: `[1, 4+num_classes, num_outputs]` — YOLOv8 transpose layout
- QNN cache dir: `/data/data/io.github.love3025.yolovaim/cache/qnn`
...
Both use uinput with virtual multitouch device (name cloned from the real touchscreen). Touch device ABS range auto-detected via `getevent -p`.
...
`app/src/main/aidl/io/github/love3025/yolovaim/IRemoteInjector.aidl` — touch injection IPC interface.
...
- QNN HTP only works on Qualcomm Snapdragon 8 Gen 1+. `yolovaim.cpp` checks `/proc/cpuinfo` for "Qualcomm" and falls back to NNAPI.
